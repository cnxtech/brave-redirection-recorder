...
const diffOriginAnchorElms = async (page, state, logger) => {
    ...
    const foundHrefs = {
        sameDomain: foundSameDomainHrefs,
        diffDomain: foundDiffDomainHrefs,
    };
...
const manualNavigation = async (page, state, logger) => {
    ...
    const anchorElms = await diffOriginAnchorElms(page, state, logger);
    const sameDomainAnchorElms = anchorElms.sameDomain;
    const diffDomainAnchorElms = anchorElms.diffDomain;
    if (sameDomainAnchorElms.length === 0 && diffDomainAnchorElms.length === 0) {
        logger(pageDesc(page) + "not navigating further, no links on page to manually visit.");
        return false;
    }

    const logMessages = [
        "First searching for anchor elements on different domains",
        "Found no anchor elements pointing to different domains, now trying on the same domain.",
    ];
    let logMessageIndex = 0;

    for (const possibleAnchorsToFollow of [diffDomainAnchorElms, sameDomainAnchorElms]) {
        logger(`${pageDesc(page)}${logMessages[logMessageIndex]}`);
        logMessageIndex += 1;

        const shuffledAnchors = shuffle(possibleAnchorsToFollow);
        const currentHost = urlsLib.getHost(page.url());

        for (const anchor of shuffledAnchors) {
            let linkHref;
            let linkFrame;
            const linkContext = await anchor.executionContext();
            try {
...
const crawl = async (crawlArgs, cb) => {
    ...
    browser.on("targetcreated", async target => {
        ...
        logger(` - ${pageDesc(newPage)}following new tab`);
