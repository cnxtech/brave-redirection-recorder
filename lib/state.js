...
const createState = _ => {
    ...
    // and this request record is a duplicate of that
    // request record.
    const addRequestRecord = requestRecord => {
        ...
        if (topEventRecord.requestedUrl !== requestRecord.url) {
            crawlEvents.push(requestRecord);
            return;
        }

        // Otherwise, dismiss the request record.
    };
    ...
    state.registerNavigationAttempt = record => {
        ...
        navigationAttempt = {
            ...
            isMainFrame: record.frameId === mainFrameId,
